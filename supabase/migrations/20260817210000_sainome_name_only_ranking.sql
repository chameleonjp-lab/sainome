...
-- 名前入力だけでランキングを登録するための台帳変更。
-- Supabase Auth（匿名ログインを含む）は使わない。
-- サーバー発行番号・時間窓・受付ゲート・重複防止は維持する。
-- 本番の受付ゲートは変更せず、停止状態を維持する。

alter table private.sainome_v2_player_keys
  add column if not exists normalized_name text;

update private.sainome_v2_player_keys
set normalized_name = public.normalize_player_name(display_name)
...
alter table private.sainome_v2_player_keys
  drop constraint if exists sainome_v2_player_keys_owner_uid_key;

create unique index if not exists sainome_v2_player_keys_normalized_name_key
  on private.sainome_v2_player_keys (normalized_name);

create index if not exists sainome_v2_plays_player_key_status_idx
  on private.sainome_v2_plays (player_key, status, expires_at);
...
begin
  ...
  if p_game_slug is null
     or p_game_slug <> 'sainome_300_seconds' then
  ...
  where plays.player_key = v_player_key
  ...
  v_earliest := v_now + interval '303 seconds';
  v_expires := v_now + interval '24 hours';

  insert into private.sainome_v2_plays (
    submission_id, owner_uid, player_key, display_name, game_slug,
    client_version, contract_version, issued_at, earliest_submit_at, expires_at
  ) values (
    v_submission_id, null, v_player_key, p_display_name, p_game_slug,
...
begin
  ...
  return query select true, v_submission_id, p_contract_version,
    p_client_version, p_game_slug, p_display_name, p_score,
    v_old.best_score, v_old.play_count, v_first, v_new_best, false;
end;
$$;
...
comment on table private.sainome_v2_player_keys is
  'Private name-key ledger for sainome-play-v2; identity is the normalized display name, not Supabase Auth.';

comment on table private.sainome_v2_plays is
  'Private server-issued play ledger for name-only sainome-play-v2; never expose through the Data API.';

comment on table private.sainome_v2_scores is
  'Private unverified aggregate for name-only sainome-play-v2; only get_sainome_ranking_v2 may read it.';

comment on function public.issue_sainome_play_v2(text, text, text, text) is
  'Issues a sainome-v2 play ID for the 300-second mode using the normalized display name; no Supabase Auth session is required.';

comment on function public.submit_score_once(text, text, integer, text, text, text) is
  'Accepts a name-matched sainome-v2 score only for a valid issued play and locked active gates; no Supabase Auth session is required.';

comment on function public.get_sainome_ranking_v2(text, integer) is
  'Returns unverified name-only sainome-v2 ranking rows; no current-user identity is available.';
