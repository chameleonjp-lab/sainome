...
- `player-name-v1`のブラウザ検査と共通テストベクトルは第2工程で実装済みである。
- Anonymous Auth、サーバー発行プレイ番号、新RPC、DB表、RLS、権限変更は第3工程で実装する。
...
1. 受付済みなら、期限後でも保存済み要求と完全一致する再送だけ、保存済み集計結果を`was_duplicate=true`で返す。
2. 受付済みで内容が異なる再送は拒否する。
3. 未使用の場合だけ、`earliest_submit_at <= DB現在時刻 < expires_at`の半開区間にあることを検査する。最短受付時刻ちょうどは許可し、失効時刻ちょうどから拒否する。
4. 未使用なら得点集計、台帳の受付済み化、完全な要求と応答の保存を一つのトランザクションで行う。
