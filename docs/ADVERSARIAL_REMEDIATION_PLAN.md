...
> 以下の工程表と詳細章には、当初のUID・プレイ番号方式を設計履歴として残している箇所がある。現行の300秒運用契約は、この冒頭の「現行300秒ランキング契約」を優先する。
...
2026年8月17日の方式決定：匿名利用者は採用しない。既存ゲームと同じく、正規化した表示名とPublishable keyだけでランキングRPCを呼び出す。現行の300秒モードでは開始時のプレイ回数加算、終了時のスコア登録、受付ゲート、結果画面のランキング取得を使う。プレイ番号・Anonymous Auth・UIDは新規プレイで使わない。同じ名前のなりすましは防げないため、本人確認が必要な用途には使わない。旧プレイ番号・UIDの記述は、移行前の設計履歴として扱う。
## 1. 目的
