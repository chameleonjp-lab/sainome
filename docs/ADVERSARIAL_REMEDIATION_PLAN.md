...
- 第7工程の自動検査を含め、`npm test`は346件全件成功した。IndexedDBの実行時失敗からlocalStorageへ切り替える経路と、切り替え先に対象がない削除を成功扱いにしない経路も検査した。DB受付とゲーム公開状態は停止したままである。iOSのタブ破棄、WebGL消失、発熱、実際のIndexedDB可用性は第11工程で確認する。
