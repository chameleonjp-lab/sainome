...
進捗：第1〜第10工程完了。第3工程は安全停止状態でDB適用・Unicode一致確認まで完了し、停止後の既発行番号を確定させない補強移行も本番へ適用済みである。拒否分類のクライアント変更はDraft実装済みで、公開有効化前の残課題は引き続き残る。第11工程の診断入口まで実装済みで、実機確認は同じ公開候補版で行う。2026年8月13日以降の新規プレイは180秒だけとし、既存の60秒保存・未送信記録は削除も変換もしない。
...
- 180秒のルールと得点式を変えない。新規60秒プレイは終了するが、既存記録は保全する。
- 新規ランキング表示は180秒の上位10名とし、過去の60秒未送信記録は元のモードのまま扱う。
...
- `supabase/migrations/20260813032103_harden_sainome_ranking_submission_contract.sql`は、発行・確定RPCを同じ公開署名のまま置き換える。未確定番号は設定行→ゲーム行→UID対応行の順にロックして受付状態を再確認し、受付済み完全一致再送だけは停止後も返す。受付状態は有効化しない。
...
- `WebGLSainome.reset()`は盤面、プレイヤー、モード、ゲーム状態を準備するが、ゲーム時間を開始しない。準備中のセッションは`idle`で、60秒または180秒の初期残り時間を保つ。
...
- `js/game-state-storage.js`がランキング保存領域と分離した`active-play-v1`へ、版付きのプレイ状態を1件だけ保存する。IndexedDBを優先し、利用できない場合は`localStorage`へフォールバックする。
...
- 起動後に保存を確認できるとホームへ「続きから再開」を表示し、明示的な「保存を削除」と完全一致確認を使う。保存を確認できない場合は上書きせず、新規開始も先に削除を要求する。保存領域のI/O障害は破損値と混同しない。
- `GameSession.restore()`と`GameFlow.resumePlaying()`で3カウントをやり直さず再開し、終了時は状態保存を削除する。ランキングの受付停止条件とSupabase本番DBは変更していない。
- 第7工程の自動検査を含め、`npm test`は309件全件成功した。DB受付とゲーム公開状態は停止したままである。iOSのタブ破棄、WebGL消失、発熱、実際のIndexedDB可用性は第11工程で確認する。
